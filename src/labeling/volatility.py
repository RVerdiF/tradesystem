...
# ---------------------------------------------------------------------------
# Volatilidade com filtro de regime
# ---------------------------------------------------------------------------
def vol_regime(
    close: pd.Series,
    span_fast: int = 10,
    ...
    close : pd.Series
        Série de preços.
    span_fast : int
        Span da volatilidade rápida.
    span_slow : int
        Span da volatilidade lenta.
    ...
    ratio = vol_fast / vol_slow.replace(0, np.nan)
    ratio.name = "vol_regime"

    logger.debug(
        "Regime de volatilidade: média ratio={:.2f}, atual={:.2f}",
        ratio.mean(),
        ratio.iloc[-1] if len(ratio) > 0 else 0.0,
    )
    return ratio
