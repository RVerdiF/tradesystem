...
from src.data.loaders import fetch_yfinance_data, fetch_mt5_data
...
@patch("src.main_execution.create_alpha_from_params")
    ...
    mock_features.return_value = pd.DataFrame(
            {
                "feat1": [1.0] * 100,
                "feat2": [2.0] * 100,
                "volume_imbalance_zscore": [1.0] * 100,
...
def test_fetch_training_data_success():
    ...
    with patch.dict(sys.modules, {"yfinance": mock_yf}):
        df = fetch_yfinance_data("PETR4.SA", years=1, interval="1h")
...
@patch("src.data.loaders.mt5_session")
@patch("src.data.loaders.extract_ohlc")
def test_fetch_mt5_training_data_success(mock_extract, mock_mt5_session):
    ...
    df = fetch_mt5_data("WINJ26", interval="1h", n_bars=100)
