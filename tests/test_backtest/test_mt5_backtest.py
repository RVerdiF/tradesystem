...
def test_fetch_mt5_data_success(mock_ohlc):
    ...
    with patch("src.main_backtest.mt5_session") as mock_session:
        with patch("src.main_backtest.extract_ohlc", return_value=mock_ohlc):
...
def test_fetch_mt5_data_empty_exit(caplog):
    ...
    with patch("src.main_backtest.mt5_session"):
        with patch("src.main_backtest.extract_ohlc", return_value=pd.DataFrame()):
