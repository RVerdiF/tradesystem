...
from src.data.extractor import extract_ohlc, extract_ticks, extract_ohlc_incremental, DataExtractionError
...
def sample_ticks():
    ...
    data = np.array([
        ...
        ('volume', '<u8'), ('flags', '<u4'), ('time_msc', '<i8'), ('volume_real', '<f8')
    ])
    return data
...
class TestDataExtractor:
    ...
    def test_extract_ticks(self, mock_mt5, sample_ticks):
        ...
        assert df.index.tz == timezone.utc
        mock_mt5.copy_ticks_range.assert_called_once_with("PETR4", start, end, mock_mt5.COPY_TICKS_ALL)
