...
def test_objective_phase1_function(sample_df, mock_results):
    ...
    trial.suggest_int.side_effect = [10, 30, 9, 21, 40, 14, 100, 30] # alpha_fast, alpha_slow, ma_fast, ma_slow, moments, atr, hurst_window, voi_window
...
def test_objective_phase1_invalid_params(sample_df):
    ...
    trial.suggest_int.side_effect = [30, 10, 9, 21, 40, 14, 100, 30] # fast=30, slow=10, ..., hurst, voi
...
def test_objective_low_trades(sample_df, mock_results):
    ...
    trial = MagicMock()
    trial.suggest_float.return_value = 0.5
    trial.suggest_int.side_effect = [10, 30, 9, 21, 40, 14, 100, 30]
