...
from src.features.indicators import rescaled_range_analysis

# Testing if we can use Numba
@njit
def fast_rs_analysis(series_values, max_lag):
    ...
    for lag in range(10, max_lag + 1, 5):
        ...
        for j in range(n_sub):
            ...
            # std with ddof=1
            s_std = np.std(chunk)
            # convert from ddof=0 to ddof=1
            if len(chunk) > 1:
                s_std = s_std * np.sqrt(len(chunk) / (len(chunk) - 1))
...
def fast_rolling_hurst(close, window=100, step=1):
    ...
    indices = range(window, len(values), step)
    for i in indices:
        window_data = values[i - window : i]
        h = fast_rs_analysis(window_data, min(window // 2, 60))
        if not np.isnan(h):
            result.iloc[i] = h
